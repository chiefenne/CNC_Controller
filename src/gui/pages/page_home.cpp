...
void page_home_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_text_font(lbl_x, &lv_font_roboto_mono_46, 0);
    lv_obj_align(lbl_x, LV_ALIGN_TOP_MID, 0, 10);

    // --- Y coordinate ---
    lbl_y = lv_label_create(parent);
    lv_label_set_text(lbl_y, "Y +000.000");
    lv_obj_set_style_text_font(lbl_y, &lv_font_roboto_mono_46, 0);
    lv_obj_align_to(lbl_y, lbl_x, LV_ALIGN_OUT_BOTTOM_MID, 0, 5);

    // --- Z coordinate ---
    lbl_z = lv_label_create(parent);
    lv_label_set_text(lbl_z, "Z +000.000");
    lv_obj_set_style_text_font(lbl_z, &lv_font_roboto_mono_46, 0);
